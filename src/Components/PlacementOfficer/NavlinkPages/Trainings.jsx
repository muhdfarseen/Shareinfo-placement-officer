import React, { useState } from 'react';
import { useDisclosure } from '@mantine/hooks';
import {
  Flex,
  Modal,
  Button,
  TextInput,
  Box,
  Pill,
  Select,
  Badge,
  Title,
  Card,
  Image,
  Group,
  ActionIcon,
  NumberInput,
  FileInput,
} from '@mantine/core';
import { DateInput } from '@mantine/dates';
import { IconSquareRoundedPlus, IconFolderCog, IconTrash } from '@tabler/icons-react';

const TrainingsCardData = [
  {
    Imgurl: "https://i0.wp.com/junilearning.com/wp-content/uploads/2020/06/python-programming-language.webp?fit=800%2C800&ssl=1",
    TrainingTitle: "Python Competitive",
    From: "09 July 2024",
    To: "09 July 2024",
    Batch: ["CS", "IT", "EC"],
    FacultyName: "Dr. Jyotika ",
    FacultyQualification: "Senior Faculty CE Trivandrum"
  },
  {
    Imgurl: "data:image/png;base64,",
    TrainingTitle: "Angular",
    From: "09 July 2024",
    To: "09 July 2024",
    Batch: ["CS", "IT", "EC"],
    FacultyName: "Dr. Jaison",
    FacultyQualification: "Senior Faculty CE Trivandrum"

  }
]


export const Trainings = () => {


  const [opened, { open, close }] = useDisclosure(false);
  const [fromdate, setFromdate] = useState(null);
  const [todate, setTodate] = useState(null);


  return (
    <div>
      <Flex justify={"space-between"} align={"center"}>
        <Title order={3}>Trainings</Title>
        <Box>
          <Button onClick={open} radius={"md"} leftSection={<IconSquareRoundedPlus size={20} />}>
            Add New Training
          </Button>
        </Box>
      </Flex>

      {/* Card looping */}

      <Box mt={20}>
        {TrainingsCardData.map((training, index) => (
          <Card mb={20} key={index} withBorder radius={"lg"}>

            <Flex justify={"space-between"}>
              <Flex gap={20}>
                <Image radius={"md"} h={90} w={90} src={training.Imgurl} />
                <Flex gap={10} direction={"column"}>
                  <Title order={5} >{training.TrainingTitle}</Title>
                  <Group>

                    <Badge radius={"sm"} variant='light'>{training.From} - {training.To}</Badge>
                    <Badge color='green' variant='light' radius={"sm"} >{training.Batch.join(", ")}</Badge>

                  </Group>
                  <Group>

                    <Pill radius={"sm"}>
                      <b>{training.FacultyName}</b> {training.FacultyQualification}
                    </Pill>

                  </Group>
                </Flex>
              </Flex>

              <Group>
                <ActionIcon size="xl" radius="lg" variant="light" aria-label="Settings">
                  <IconFolderCog style={{ width: '70%', height: '70%' }} stroke={1.5} />
                </ActionIcon>
                <ActionIcon color='red' size="xl" radius="lg" variant="light" aria-label="Settings">
                  <IconTrash style={{ width: '70%', height: '70%' }} stroke={1.5} />
                </ActionIcon>
              </Group>
            </Flex>

          </Card>
        ))}
      </Box>

      {/* Modal content */}

      <Modal opened={opened} onClose={close} title="Add New Training">
        <Flex direction={"column"} gap={20}>

          <TextInput label="Programme Name" />
          <FileInput label="Training Icon Image" />

          <Group grow>
            <DateInput value={fromdate} onChange={setFromdate} label="From" />
            <DateInput value={todate} onChange={setTodate} label="To" />
          </Group>

          <TextInput label="Faculty Name" />
          <TextInput label="Faculty Qualification" />

          <FileInput label="Upload Syllabus" />

          <Button fullWidth>Publish Training</Button>

        </Flex>
      </Modal>

    </div>
  )
}
